...
functions/
  new.js          GET /new — What's New, rendered here, no JavaScript
  api/ask.js      THE worker: one route, one flow
  api/scan.js     POST /api/scan — fills the grid, token-guarded
src/
  blog.js         What's New: the sheet's other projection, pure
...
| `blog` | `/new` | the row shows on What's New |
...
`/new` also reads `date`, `format` and `pin`. Rows written before those columns
...
`/*` block either. `/new` is the first route here that is a Function rather
