...
*Τρέξιμο tests:* `node --test test/router.test.mjs test/scan.test.mjs
test/sheet.test.mjs` (το `node --test test/` δεν δουλεύει σε αυτό το Node —
θέλει αρχεία, όχι φάκελο).
...
- [ ] **Creator-scoped queries** — «τι έχεις από τον X;», «Y για το θέμα Z».
      Σήμερα ο bot ματσάρει μόνο θέμα· η ερώτηση «κατά creator» δεν έχει
      μονοπάτι. **Επόμενο build, με σχεδιασμό πρώτα** — όχι patch πάνω στο
      topic-matching. Ο κανόνας που ξέρουμε ήδη από τα δεδομένα: ένα βίντεο με
      **δύο creators** (οι μισές γραμμές του Sheet είναι «Ken Berry + κάποιος»)
      **μετράει και για τους δύο** — το πεδίο `creator` είναι ελεύθερο κείμενο,
      άρα θα θέλει δομή πριν γίνει ερωτήσιμο. Προσοχή στο §14.5: «τι έχεις από
      τον X» είναι *πρόσβαση κατά πρόσωπο*, όχι κατάταξη προσώπων.
- [ ] **Τρία νέα θέματα: `sugar`, `blood-pressure`, `fibre`** — curation entries
      + aliases (EN + EL), όπως έγινε με το `randle`. Θέλουν και ανάθεση
      creators, που είναι απόφαση του Nick (§14.3).
- [ ] **Μικρά UX** (μαζεμένα, όχι επείγοντα): clickable pills για τα θέματα·
      toggle EN/EL **και link προς το portal**· ο Dave Mac να εμφανίζεται με
