...
- [ ] **Το 502 να μη μοιάζει με σπασμένο site.** Στο incident του Mistral
      (05/09/2026) το production έβγαζε **γυμνή σελίδα σφάλματος της
      Cloudflare** αντί για το δικό μας φιλικό JSON — ο worker επιστρέφει
      `{ error: 'upstream_unavailable' }` με 502, αλλά ο επισκέπτης έβλεπε το
      generic error page. **Outage δεν πρέπει να διαβάζεται ως εγκατάλειψη:**
      το κοινό είναι 50+ και ένα Cloudflare error page λέει «χάλασε το site»,
      ενώ η αλήθεια είναι «ο πάροχος του μοντέλου δεν απαντά τώρα, ξαναδοκίμασε
      σε λίγο». Θέλει να δούμε **τι ακριβώς βλέπει ο χρήστης** (ο client τι
      κάνει με 502/503, και αν παρεμβαίνει το Cloudflare error handling πάνω
      από τη response μας) πριν αποφασιστεί λύση. **Σημειωμένο, όχι
      δουλεμένο** — δική του δουλειά, όχι παρελκόμενο του creator-scoped.
