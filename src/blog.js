/* ---------------------------------------------------------------------------
   What's New — the curated list, read from the SAME sheet the override reads.
   ...
   override layer and the override layer does not know it exists:
     ...
     · `blog`   is this page's switch. It decides what What's New shows.
   ...
   cache and the Response live in functions/new.js, so all of this runs under
 ...
 * page on this site gets its policy from that file; /new cannot, so it carries
 * its own — including the headers the `/*` block hands to everything else,
 * because that block does not reach here either.
 *
 * Left out, /new would ship with no CSP AND no frame-ancestors, which does not
 * fail loudly: the portal's iframe would work perfectly while every other site
 * on the internet could frame this page too. test/blog.test.mjs exists to keep
...
/** "2026-08-30" → "30 Aug 2026". The page is English, like /embed. */
...
function card(item, topics) {
  ...
  const meta = [item.creator, item.duration, displayDate(item.date)].filter(Boolean);
  const pills = [
...
<title>What's New — Ask Carnivore Ai</title>
<meta name="description" content="Recently added videos from the creators this bot draws on.">
...
<body>
  <div class="wrap">
    <header class="head">
      <h1>What's New</h1>
      <p>Recently added to the index — picked by hand, newest first.</p>
    </header>
